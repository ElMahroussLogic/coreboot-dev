...
{
    mp_base_tree = (struct hw_mp_pci_tree*)(MP_PCI_TREE_BASE);

    // huh? anyway let's ignore it then.
    if (mp_base_tree->d_magic == SYS_PCI_DEV_MAGIC)
    {
        mp_panic(">> Trying to overwrite an existing pci tree.\n");
        return no;
    }
...
boolean mp_pci_append_tree(const caddr_t name, mp_pci_num_t struct_ptr, mp_pci_num_t struct_sz)
{
    ...
    struct hw_mp_pci_tree* mp_pci_tree = (struct hw_mp_pci_tree*)(mp_latest_tree + sizeof(struct hw_mp_pci_tree));
