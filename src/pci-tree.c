...
static volatile struct hw_mp_pci_tree* mp_last_tree = nil;
...
boolean mp_pci_init_tree(void)
{
	...
	memncpy(mp_base_tree->d_name, PCI_TREE, strlen(PCI_TREE));
...
boolean mp_pci_append_tree(const caddr_t name, mp_pci_num_t struct_ptr, mp_pci_num_t struct_sz)
{
    ...
    volatile struct hw_mp_pci_tree* mp_pci_tree = (volatile struct hw_mp_pci_tree*)(mp_latest_tree);

    while (mp_pci_tree->d_magic == SYS_PCI_DEV_MAGIC) {
	if (strcmp(mp_pci_tree->d_name, name) == 0)
		return no;

	mp_pci_tree = (volatile struct hw_mp_pci_tree*)(mp_pci_tree  + sizeof(struct hw_mp_pci_tree));
    }
    ...
    mp_pci_tree->d_next_sibling = (mp_pci_num_t)(mp_pci_tree + sizeof(struct hw_mp_pci_tree));
